...
export class YoutubeService {
    ...
    public authorize(requestData: any, token: any) {
        var credentials = {
     ...
     */
    public removeEmptyParameters(params: any) {
        for (let p in params) {
    ...
    public activitiesList(auth: any, requestData: any) {
        var service = google.youtube('v3');
        var parameters = this.removeEmptyParameters(requestData['params']);
        parameters['auth'] = auth;
        service.activities.list(parameters, function(err: any, response: any) {
            if (err) {
